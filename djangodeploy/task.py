...
def _chown_user():
    sudo('chown -R  django:www-data %(code_root)s' % env)
...
def _upload_postgres_conf():
    ''' upload rungunicorn conf '''
    if isfile('conf/pg_hba.conf'):
        ''' we use user defined rungunicorn file '''
        template = 'conf/pg_hba.conf'
    upload_template(template, env.postgres_conf,
                    context=env, backup=False, use_sudo=True)
    sudo('chown postgres:postgres %s' % (env.postgres_conf))
...
def _install_django():
    virtenvsudo('pip install django')


def _install_pillow():
    virtenvsudo('pip install -I --no-cache-dir -v Pillow')
...
def _setup_directories():
    ...
    sudo('chmod -R 755 %s' % dirname(env.supervisor_stdout_logfile))
...
# def _migrate_db():
#     sudo('cd %s/%s' % (env.code_root, env.add_app)
#     sudo('pthon manage.py migrate')
...
def _create_db_and_user():
     # a = "CREATE USER %s WITH NOCREATEDB NOCREATEUSER ENCRYPTED %s " % (env.psql_user , env.psql_pass)
     # sudo("psql -c %s , user='postgres'" % a)
     # b = "CREATE DATABASE %s WITH OWNER %s" % (env.psql_db , env.psql_user)
     # sudo("psql -c %s, user='postgres'" % b)
    sudo('psql -U postgres -c "CREATE ROLE {0} WITH PASSWORD \'{1}\' NOSUPERUSER CREATEDB NOCREATEROLE LOGIN;"'.format(env.psql_user, env.psql_pass))
...
def _prepare_django_project():
    with cd(env.django_project_root):
        virtenvsudo('python manage.py migrate auth')
        #virtenvsudo('python manage.py makemigrations')
        virtenvsudo('python manage.py migrate')
        #virtenvsudo('./manage.py syncdb --noinput --verbosity=1')
        if env.south_used:
            #virtenvsudo('./manage.py migrate --noinput --verbosity=1')
            pass
